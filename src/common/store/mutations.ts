import { LOAD_ACCOUNTS, LOAD_ACCOUNT, LOAD_FEED, LOAD_POSTS, CAST_VOTE, LOAD_TRENDING } from './constants';
...
store.registerAction(LOAD_POSTS, loadPosts);
store.registerAction(LOAD_ACCOUNTS, loadAccounts);
store.registerAction(LOAD_ACCOUNT, loadAccount);
store.registerAction(LOAD_FEED, loadFeed);
store.registerAction(CAST_VOTE, castVote);
store.registerAction(LOAD_TRENDING, loadTrending);
